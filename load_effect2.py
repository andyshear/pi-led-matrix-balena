...
def effect_times(_initial_rider_data_ignored):
    ...
    y0 = -2                 # top line y
    y1 = y0 + line_h + LINE_GAP  # bottom line y
    ...
    def make_surfaces(entry, dim: bool):
        ...
        w_bot = dd.textbbox((0, 0), lap_time,  font=font)[2]
        top_w = max(pane_w, w_top) + 2  # top can scroll beyond pane
        # bottom is fixed to pane width
        ...
        # left-align lap time; keep your -1 pixel tweak
    ...
    while not stop_event.is_set() and get_current_effect() == 'times':
        ...
        # include laps/lap_time in keys so we rebuild if either changes
        ...
        if this_cur_key != cur_key:
            ...
            # reset scroll if new content is narrower than pane
        ...
        # LEFT PANE (current): paste scrolling top + static bottom
