...
def apply_effect(effect_name, payload=None):
    ...
    if effect_name == 'times' and get_current_effect() == 'times' and current_effect_thread is not None and current_effect_thread.is_alive():
        if payload is not None:
            print(f"[times] queue existing payload type={type(payload).__name__} payload={payload}")
    ...
    if effect_name == 'times' and payload is not None:
        print(f"[times] queue initial payload type={type(payload).__name__} payload={payload}")
        times_queue.put(payload)
...
def listen_for_commands():
    while True:
        ...
        try:
            ...
            print(f"[cmd] effect={effect_name} rider_data_type={type(rider_data).__name__} rider_data={rider_data}")
